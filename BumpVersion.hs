...
#! nix-shell --pure -i runghc -p haskellPackages.hpack jq yq git "haskellPackages.ghcWithPackages (hp: with hp; [ shh optparse-generic say ])"
...
main = do
  ...
  missingExecutables >>= \case
    [] -> pure ()
    xs -> do
      for_ xs $ \x -> sayErrString ("missing executable " <> x)
      exitFailure
